    ...
    <div className="page-shell updates-page">
      ...
      <div className="updates-layout">
        <div className="updates-list">
          {updates.map((update) => (
            <article key={`${update.item.slug}-${update.date}-${update.text}`}>
              <time>{update.date}</time>
              <div>
                <span className={`status-pill status-${update.item.status.tone}`}><span className="status-dot" />{update.item.status.label}</span>
                <h2><Link href={`/exams/${update.item.slug}`}>{update.item.shortTitle}</Link></h2>
                <p>{update.text}</p>
                <a href={update.item.sourceUrl} target="_blank" rel="noreferrer">Official source ↗</a>
              </div>
            </article>
          ))}
        </div>
        <aside className="update-method-card">
